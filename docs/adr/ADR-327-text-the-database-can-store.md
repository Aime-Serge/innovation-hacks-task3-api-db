...
text value containing a NUL character (`\u0000`) or a lone surrogate (`\ud800` to `\udfff`) cannot be
stored by PostgreSQL. Both are refused at the edge now, with a 422, and the schema says so:

- Every free-text request field (names, titles, descriptions, search text, email, password, avatar
  URL) carries a `pattern` that excludes them. The name and title patterns also still require at
